...
class OpenPNE_ImgCrop
{
     ...
     * @param array $options
     */
    function OpenPNE_ImgCrop($options = array())
    {
    ...
    function generate_img()
    {
        ...
        //リサイズ
        $output_img = $this->resize_img($source_gdimg, $w, $h, $this->requests['thumbnail']);
    ...
    function resize_img($source_gdimg, $w, $h, $thumbnail)
    {
        ...
        // トリミング
        if ($thumbnail == 'fitCrop' && $s_width != $s_height) {
            // トリミング出力先
    ...
    function set_cache_filename()
    {
        ...
        $thumbnail = $this->requests['thumbnail'];

        $this->cache_fullpath =
                 $this->cache_dir . '/' .
                 $this->get_cache_path($filename, $w, $h, $f, $thumbnail);
    }
     ...
     * static
     */
    function get_cache_path($filename, $w, $h, $f, $thumbnail)
    {
        ...
        if ($thumbnail) {
            $path = "{$f}/w{$w}_h{$h}/$thumbnail/{$prefix}{$file}";
        } else {
