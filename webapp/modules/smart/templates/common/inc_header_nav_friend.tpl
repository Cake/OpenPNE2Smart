...
<li id="navFriendHome" class="ui-block-c"><a href="({t_url m=pc a =page_f_home})&amp;c_member_id=({$target_member.c_member_id})" class="({if $INC_HEADER_page_name == 'f_homet'}) ui-btn-active({/if})" data-role="button" data-corners="false" data-mini="true">({$WORD_MY_FRIEND_HALF})ﾎｰﾑ</a></li>
</div>({* navFriend }}} *})
