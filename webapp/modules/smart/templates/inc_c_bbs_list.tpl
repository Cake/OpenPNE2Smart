...
({if isset($item.write_num) || isset($item.max_number)})<span class="num_comment">コメント(({if isset($item.write_num)})({$item.write_num})({elseif isset($item.max_number)})({$item.max_number})({elseif isset($item.number)})({$item.number})({/if}))</span>({/if})
