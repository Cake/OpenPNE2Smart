...
<div class="photo48"><img class="ui-li-thumb memberPhoto" src="({t_img_url filename=$commu_image w=48 h=48 noimg=no_image trim=square})" alt="プロフィール写真"></div>
<div class="itemData">
...
({if isset($item.write_num) || isset($item.max_number) || isset($item.number) || isset($item.count_comments)})<span class="num_comment">コメント(({if isset($item.write_num)})({$item.write_num})({elseif isset($item.max_number)})({$item.max_number})({elseif isset($item.number)})({$item.number})({elseif isset($item.count_comments)})({$item.count_comments})({/if}))</span>({/if})
({if $item.image_filename1||$item.image_filename2||$item.image_filename3})<span class="footer-icon">({t_smart_icon_url icon_name="smart-camera"})</span>({/if})
</div></div>
</section></a>
</li>
