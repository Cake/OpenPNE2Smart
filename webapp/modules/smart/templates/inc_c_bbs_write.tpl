...
<div class="photo48"><a href="({t_url m=pc a=page_f_home})&amp;target_c_member_id=({$item.c_member_id})"><img class="ui-li-thumb memberPhoto" src="({t_img_url filename=$item.image_filename w=48 h=48 noimg=no_image})" alt=""></a></div>
<div class="itemData">
...
({if $item.image_filename1})<li class="ui-block-a"><a href="({t_img_url filename=$item.image_filename1})" data-transition="pop" data-ajax="false"><img src="({t_img_url filename=$item.image_filename1 w=76 h=76})"></a></li>({/if})
({if $item.image_filename2})<li class="ui-block-b"><a href="({t_img_url filename=$item.image_filename2})" data-transition="pop" data-ajax="false"><img src="({t_img_url filename=$item.image_filename2 w=76 h=76})"></a></li>({/if})
({if $item.image_filename3})<li class="ui-block-c"><a href="({t_img_url filename=$item.image_filename3})" data-transition="pop" data-ajax="false"><img src="({t_img_url filename=$item.image_filename3 w=76 h=76})"></a></li>({/if})
</ul>({/if})
